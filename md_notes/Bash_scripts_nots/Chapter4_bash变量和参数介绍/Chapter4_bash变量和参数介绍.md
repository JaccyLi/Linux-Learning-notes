...
exit 0
```
- 下面花括号内的位置变量（使用${!val}的形式）使得引用最后一个从命令行传给脚本的位置参数非常方便，这需要间接引用。
...
- **某些脚本由于调用它们的名字不同，会表现出不同的行为。要使得这种用法可行，脚本需要检查$0这个位置参数，即调用它的名字。这些可选的名字必须是不同得符号链接（symbolic links）。**
...
case `basename $0ìn    # Or:    case ${0##*/} in
