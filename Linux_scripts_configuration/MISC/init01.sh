...
if [ $? -eq 0 ]; then
   sleep 0.5
    echo -e "第四项，成功设置CD为yum源^_^`success`" 
else
    echo -e "第四项，设置CD为yum源失败:(`failure`"
fi
sleep 1
echo
...
[ $? -eq 0 ] && echo -e "成功安装chrony`success`" || echo -e "chrony安装失败`failure`"
yum -y install wget tree &> /dev/null
[ $? -eq 0 ] && echo -e "成功安装wget,tree`success`" || echo -e "wget,tree安装失败`failure`"
yum -y install screen net-tools &> /dev/null
[ $? -eq 0 ] && echo -e "成功安装screen net-tools `success`" || echo -e "screen net-tools安装失败`failure`"
yum -y install  vim bzip2 &> /dev/null
[ $? -eq 0 ] && echo -e "成功安装vim,bzip2`success`" || echo -e "vim,bzip2安装失败`failure`"
sleep 2
echo 
...
[ $? -ne 0 ] && echo "第六项，关闭防火墙失败`failure`" 
systemctl disable firewalld &> /dev/null
[ $? -ne 0 ] && echo "disable firewalld fail"
iptables -F &> /dev/null
[ $? -ne 0 ] && echo "shutdown iptables fail"
echo "第七项,最后一项，失能selinux..."
sed -r -i 's#(^SELINUX=)(.*$)#\1disabled#' /etc/selinux/config &> /dev/null
[ $? -ne 0 ] && echo "失能selinux失败`failure`" || echo "失能selinux成功`success`"
